...
def _mayhemfile_impl(ctx):
    ctx.actions.expand_template(
        ...
        substitutions = {
            ...
            "{TASKS}": "tasks: \n".format("\n".join(' - name: {}'.format(v) in ctx.attr.tasks) if ctx.attr.tasks else "",
            "{TESTSUITE}": "testsuite: \n".join(' - {}'.format(v) in ctx.attr.testsuite) if ctx.attr.testsuite else "",
            ...
            "{ENV}": ctx.attr.env if ctx.attr.env else "",
            ...
            "{NETWORK}" : "network: \n".format("\n".join(' - {}: {}'.format(v) in **ctx.attr.network) if ctx.attr.network else "",
            "{MAX_LENGTH}": "max_length: {}".format(ctx.attr.max_length) if ctx.attr.max_length else "",
            "{TIMEOUT}": "timeout: {}".format(ctx.attr.timeout) if ctx.attr.timeout else "",
...
_mayhemfile = rule(
    ...
    attrs = {
        ...
        "tasks": attr.string(mandatory = False),
        ...
        "env": attr.string(mandatory = False),
        ...
        "network": attr.string(mandatory = False),
        "max_length": attr.string(mandatory = False),
        "timeout": attr.string(mandatory = False),
